...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
     ...
     *
     * - anyRequest()
     * 설정된 값들 이외 나머지 URL들을 나타낸다. 여기에는 authenticated()을 추가하여
     * 나머지 URL들은 모두 인증된 사용자들에게만 허용하게 한다.
     *
     * - logout().logoutSuccessUrl("/")
     * 로그아웃 기능에 대한 여러 설정의 진입점이다. 로그아웃 성공 시 / 주소로 이동한다.
     *
     * - oauth2Login()
     ...
     * 소셜 로그인 성공 시 후속 조치를 진행할 UserService 인터페이스의 구현제를 등록한다.
